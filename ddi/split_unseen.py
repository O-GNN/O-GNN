...
target_dir = os.path.join(os.path.dirname(__file__), "data", "unseen")
ssiddi_dir = os.path.join(os.path.dirname(__file__), "data", "ssiddi")
...
def check_ilegal_negsample():
    ...
    keep_error = 0
    for d1, d2, n in zip(
        tqdm(ind_train_csv["Drug1_ID"]), ind_train_csv["Drug2_ID"], ind_train_csv["Neg samples"],
    ):
        d_neg, pos = n.split("$")
        if pos == "h":
    ...
    ind_valid_csv = pd.read_csv(ind_valid)
    for d1, d2, n in zip(
        tqdm(ind_valid_csv["Drug1_ID"]), ind_valid_csv["Drug2_ID"], ind_valid_csv["Neg samples"],
    ):
        d_neg, pos = n.split("$")
...
def check_new_build():
    ...
    for d1, d2 in zip(tqdm(ind_valid_csv["id1"]), ind_valid_csv["id2"]):
        assert d1 not in drug_set and d2 not in drug_set
        drug_pair.add((d1, d2))
        drug_pair.add((d2, d1))

    for nd1, nd2 in zip(tqdm(ind_valid_csv["neg_id1"]), ind_valid_csv["neg_id2"]):
        assert nd1 not in drug_set and nd2 not in drug_set
        assert (nd1, nd2) not in drug_pair
...
def rebuild_drug_pair():
    ...
    for file in [ind_train, ind_valid, ind_test]:
        ...
        for d1, d2 in zip(tqdm(file_csv["Drug1_ID"]), file_csv["Drug2_ID"]):
            ...
            if file == ind_test:
                test_drug_dict.add(d1)
                test_drug_dict.add(d2)
    ...
    reb_dir = "{}/{}".format(target_dir, "new_build2")
    ...
    all_drug_set = test_drug_dict
    valid_data = []
    valid_csv = pd.read_csv(ind_test)
    for d1, d2, y in zip(tqdm(valid_csv["Drug1_ID"]), valid_csv["Drug2_ID"], valid_csv["Y"]):

        assert d1 not in train_drug_dict and d2 not in train_drug_dict
        if np.random.rand() > 0.5:
            neg_d1 = d1
            candidate_list = list(all_drug_set - drug_dict[d1])
            candidate_idx = np.random.choice(len(candidate_list), 1)[-1]
...
def build_raw():
    ...
    train_data = []
    train_csv = pd.read_csv(ind_train)
    for d1, d2, y, neg in zip(
        tqdm(train_csv["Drug1_ID"]), train_csv["Drug2_ID"], train_csv["Y"], train_csv["Neg samples"]
    ):
    ...
    valid_csv = pd.read_csv(ind_test)
    for d1, d2, y, neg in zip(
        tqdm(valid_csv["Drug1_ID"]), valid_csv["Drug2_ID"], valid_csv["Y"], valid_csv["Neg samples"]
    ):
        ...
        assert d1 not in training_set and d2 not in training_set

        if neg_d1 in training_set:
            error_count += 1
            print(neg_d1)
        if neg_d2 in training_set:
            error_count += 1
            print(neg_d2)
